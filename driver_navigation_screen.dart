...
class DriverNavigationScreen extends StatefulWidget {
  ...
  final LatLng? driverPosition; // ← passed from dashboard (real GPS)
...
class _DriverNavigationScreenState extends State<DriverNavigationScreen> {
  ...
  // Patient position — comes from request data when backend is connected.
  // Falls back to a fixed Cebu City point for now.
  LatLng get _patientPos {
    final lat = widget.request['pickup_latitude'] as double?;
    final lng = widget.request['pickup_longitude'] as double?;
    if (lat != null && lng != null) return LatLng(lat, lng);
    return const LatLng(10.3157, 123.8854); // fallback
  }

  LatLng get _mapCenter => LatLng(
        (_driverPos.latitude + _patientPos.latitude) / 2,
        (_driverPos.longitude + _patientPos.longitude) / 2,
      );

  // ── Distance & ETA (simple Haversine estimate) ─────────────────────────────
  ...
  String get _distanceLabel => '${_distanceKm.toStringAsFixed(1)} km';

  String get _etaLabel {
    // Rough ETA: assume 30 km/h average in city
    final minutes = (_distanceKm / 30 * 60).ceil();
  ...
  void initState() {
    super.initState();
    // Use position passed from dashboard, or fallback to Cebu center
    _driverPos = widget.driverPosition ?? const LatLng(10.3220, 123.8920);
    _startTracking();
  }
  ...
  void _startTracking() {
    _locationSub = LocationService.instance.startTracking().listen((position) {
      ...
      // Push live location to server so patient can see driver moving
      TrackingService.instance.pushDriverLocation(
        latitude: position.latitude,
        longitude: position.longitude,
        activeRequestId: widget.request['id']?.toString(),
      );
    });
  }
                            ...
                            children: [
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  Text(_etaLabel, style: GoogleFonts.outfit(fontSize: 17, fontWeight: FontWeight.w900, color: AppTheme.warning)),
                                ],
                              ),
                            ],
                          ),
                            ...
                            children: [
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  Text(_distanceLabel, style: GoogleFonts.outfit(fontSize: 17, fontWeight: FontWeight.w900, color: AppTheme.blue)),
                                ],
                              ),
                            ],
                          ),
